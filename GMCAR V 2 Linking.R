...
rho_sc_posterior <- function(rho1, rho2, mu1, mu2, tau1, tau2, eta0, eta1, Adj, Minc) {
  n <- nrow(Adj)
  I <- diag(n)
  D <- diag(rowSums(Minc))
  ...
  for (i in 1:length(rho1)) {
    ...
    W <- Adj
    E_1 <- solve(tau1[i] * (D - rho1[i] * W)) + (eta0[i] * I + eta1[i] * W) %*% solve(tau2[i] * (D - rho2[i] * W)) %*% (eta0[i] * I + eta1[i] * W)
    E_12 <- (eta0[i] * I + eta1[i] * W) %*% solve(tau2[i] * (D - rho2[i] * W))
...
# Read spatial data: Valparaiso Region 
rds_valparaiso <- readRDS("/Users/clementeferrer/Documents/Paper CAR/Datos/gadm41_CHL_3_pk.rds")
...
ca.neighbors <- poly2nb(rds_valparaiso)
n <- length(ca.neighbors)
Adj <- sapply(ca.neighbors,function(x,n) {v=rep(0,n);v[x]=1;v},n)

# Read poverty data
datos_v <- read_xlsx("/Users/clementeferrer/Documents/Tesis USM/Datos/datos pobreza_v.xlsx")
Y1 = as.numeric(datos_v$HT)
Y2 = as.numeric(datos_v$Casen)
...
cat("
    ...
    beta[1:nc] ~ dmnorm(rep(0.126,nc), (0.1*I1))
    }
    ", fill = TRUE)
sink()
...
model.data2 <- list(k = n, nc = ncol(X.o2), I = diag(n), I1 = diag(ncol(X.o2)),
                    Minc = Minc, D = diag(rowSums(Minc)), X = X.o2, Y = Y.o2)

model.inits <- rep(list(list(rho1 = 0.5, rho2 = 0.5, tau1 = 1, tau2 = 1, eta021 = 0, 
                             eta121 = 0, taue1 = 1, taue2 = 1,
                             beta = rep(0.126, ncol(X.o1)), W1 = rep(0, n), 
                             W2 = rep(0, n))),1)
...
rho_sc_values <- rho_sc_posterior(rho1, rho2, mu1, mu2, tau1, tau2, eta0, eta1,
                                  Adj, Minc)
...
HPDinterval(as.mcmc(mu1), prob = 0.95)
HPDinterval(as.mcmc(mu2), prob = 0.95)
HPDinterval(as.mcmc(tau1), prob = 0.95)
HPDinterval(as.mcmc(tau2), prob = 0.95)
HPDinterval(as.mcmc(rho1), prob = 0.95)
HPDinterval(as.mcmc(rho2), prob = 0.95)
HPDinterval(as.mcmc(eta0), prob = 0.95)
HPDinterval(as.mcmc(eta1), prob = 0.95)

HPDinterval(as.mcmc(rho_sc_values), prob = 0.95)
mean(rho_sc_values)

...
rho_sc_posterior <- function(rho1, rho2, mu1, mu2, tau1, tau2, eta0, eta1, eta2, eta3, Adj, Adj2, Adj3, Minc) {
  n <- nrow(Adj)
  I <- diag(n)
  D <- diag(rowSums(Minc))
  ...
  for (i in 1:length(rho1)) {
    ...
    W2 <- Adj2
    W3 <- Adj3
    E_1 <- solve(tau1[i] * (D - rho1[i] * W)) + (eta0[i] * I + eta1[i] * W + eta2[i] * W2 + eta3[i] * W3) %*% solve(tau2[i] * (D - rho2[i] * W)) %*% (eta0[i] * I + eta1[i] * W + eta2[i] * W2 + eta3[i] * W3)
    E_12 <- (eta0[i] * I + eta1[i] * W + eta2[i] * W2 + eta3[i] * W3) %*% solve(tau2[i] * (D - rho2[i] * W))
...
# Create third order neighbors matrix
Adj3 <- Adj %*% Adj %*% Adj
Adj3[Adj > 0] <- 0
Adj3[Adj2 > 0] <- 0 
diag(Adj3) <- 0 
...
Y1 = as.numeric(sqrt(datos_v$HT))
Y2 = as.numeric(sqrt(datos_v$Casen))
...
cat("
    ...
    A21 <- eta021 * I + eta121 * Minc + eta221 * Minc2 + eta321 * Minc3
